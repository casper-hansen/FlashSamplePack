...
from flash_sample_pack.attention_utils import get_cu_seqlens_from_pos_ids


def patch_llama_derived_model():
...
def flashattn_forward(
    ...
    if self.pretraining_tp > 1:
        ...
        query_states = [
            torch.nn.functional.linear(hidden_states, query_slices[i])
            for i in range(self.pretraining_tp)
        ]
        query_states = torch.cat(query_states, dim=-1)

        key_states = [
            torch.nn.functional.linear(hidden_states, key_slices[i])
            for i in range(self.pretraining_tp)
        ]
        key_states = torch.cat(key_states, dim=-1)

        value_states = [
            torch.nn.functional.linear(hidden_states, value_slices[i])
            for i in range(self.pretraining_tp)
        ]
    ...
    if self.pretraining_tp > 1:
        ...
        attn_output = sum(
            torch.nn.functional.linear(attn_output[i], o_proj_slices[i])
            for i in range(self.pretraining_tp)
        )
    else:
